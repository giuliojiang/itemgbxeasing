export function findMesh(decomp){
  ...
  for(let off=0;off<len-4-60*12;off+=4){
    ...
    for(let i=0;i<Math.min(cnt,60);i++){
      ...
      if(Math.abs(x)>100||Math.abs(y)>100||Math.abs(z)>100){ok=false;break;}
      if(x<minx)minx=x; if(y<miny)miny=y; if(z<minz)minz=z;
      if(x>maxx)maxx=x; if(y>maxy)maxy=y; if(z>maxz)maxz=z;
    }
    if(!ok||zero>35) continue;
    const size=Math.max(maxx-minx,maxy-miny,maxz-minz);
    if(size<0.15||size>25) continue;
    if(Math.min(maxx-minx,maxy-miny,maxz-minz)<size*0.015&&cnt>300) continue;
    const after=posOff+cnt*12;
    if(after+4>len) continue;
    const edgeCnt=dv.getInt32(after,true);
    if(edgeCnt<cnt*0.3||edgeCnt>cnt*4) continue;
    ...
    if(tris.length<cnt*0.25||tris.length>cnt*4||tris.length<15) continue;
    ...
    if(valid<Math.min(4,tris.length)) continue;
  ...
  for(let off=0;off<len-12;off+=4){
    for(let stride of [12,24,32]){
      ...
      while(o+12<=len&&cnt<12000){
        ...
        if(Math.max(maxx-minx,maxy-miny,maxz-minz)>30) break;
        cnt++; o+=stride;
      }
      if(bad) continue;
      if(cnt<60) continue;
  ...
  for(let s=0;s<len-16;s++){
    ...
    if(isNew){
      for(let i=0;i<count;i++){const d=dv.getInt16(p+8+i*2,true); cur+=d; if(cur<0||cur>30000){ok=false;break;} indices[i]=cur; if(cur>maxI) maxI=cur;}
    }else{
      for(let i=0;i<count;i++){const idx=dv.getUint16(p+8+i*2,true); if(idx>30000){ok=false;break;} indices[i]=idx; if(idx>maxI) maxI=idx;}
    }
    if(!ok||maxI<10) continue;
    ibs.push({offset:s,count,maxI,indices,isNew});
    if(ibs.length>=12) break;
  }
  ...
  for(let ib of ibs){
    for(let stride of [12,24]){
      for(let gap of gaps){
        ...
        if(!ok) continue;
        const size=Math.max(maxx-minx,maxy-miny,maxz-minz);
        if(size<0.1||size>35) continue;
        const tryOffs=stride===24?[0,12]:[0];
        for(let po of tryOffs){
          let valid=0;
          for(let i=0;i<Math.min(ib.count-2,90);i+=3){const a=ib.indices[i],b=ib.indices[i+1],c=ib.indices[i+2]; if(a>=vc||b>=vc||c>=vc) continue; if(a===b||b===c||a===c) continue; const ar=triArea(off,stride,a,b,c,po); if(ar>1e-7&&ar<50) valid++;}
          if(valid<3) continue;
          const score=valid*1000 - Math.log2(gap+2)*20;
    ...
    for(let cand of candidates){
      ...
      if(cand.offset+cand.vertCount*cand.stride>ib.offset+5000) continue;
      if(ib.offset - (cand.offset+cand.vertCount*cand.stride) > 150000) continue;
      const tryOffs=cand.stride===24?[0,12]:[0];
      for(let po of tryOffs){
        let valid=0;
        for(let i=0;i<Math.min(ib.count-2,90);i+=3){const a=ib.indices[i],b=ib.indices[i+1],c=ib.indices[i+2]; if(a>=cand.vertCount||b>=cand.vertCount||c>=cand.vertCount) continue; if(a===b||b===c||a===c) continue; const ar=triArea(cand.posOffset,cand.stride,a,b,c,po); if(ar>1e-7&&ar<50) valid++;}
        if(valid<3) continue;
        const gap=ib.offset-(cand.offset+cand.vertCount*cand.stride);
        const score=valid*800 - Math.abs(gap)*0.01 - (cand.vertCount/(ib.maxI+1))*5;
  ...
  // 3. Legacy – try any ib near candidate (wider range)
  for(let cand of candidates){
    for(let s=Math.max(0,cand.offset+cand.vertCount*cand.stride-2000); s<Math.min(len-6,cand.offset+cand.vertCount*cand.stride+15000); s+=2){
      if(s+4>len) continue;
      const cnt=dv.getUint16(s,true);
      if(cnt<30||cnt>20000||cnt%3!==0) continue;
      ...
      if(!ok) continue;
      let valid=0; for(let i=0;i<Math.min(cnt-2,45);i+=3){const a=dv.getUint16(s+4+i*2,true),b=dv.getUint16(s+4+i*2+2,true),c=dv.getUint16(s+4+i*2+4,true); if(a===b||b===c||a===c) continue; const ar=triArea(cand.posOffset,cand.stride,a,b,c,0); if(ar>1e-7&&ar<50) valid++;}
      if(valid<3) continue;
